...
defineProps<{ items: CartUiItem[]; subtotalOriginal: number; total: number; disabled?: boolean; discount: number; deliveryFee: number; methodLabel: string }>()
...
<template>
  <aside class="rounded-2xl border border-border bg-card p-5 sm:p-6">
    ...
    <div class="flex gap-2 overflow-hidden py-4"><div v-for="item in items.slice(0, 4)" :key="item.id" class="relative size-11 shrink-0 overflow-hidden rounded-lg bg-surface"><img v-if="item.imageUrl" :src="item.imageUrl" :alt="item.name ?? ''" class="size-full object-cover" /><span class="absolute bottom-0 inset-e-0 grid size-4 place-items-center rounded-ss bg-primary text-[9px] text-primary-foreground">{{ item.quantity }}</span></div></div>
