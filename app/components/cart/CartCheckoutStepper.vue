...
const steps = [
  { id: 1, label: 'سبد خرید', icon: 'solar:cart-large-2-outline' },
  { id: 2, label: 'مشخصات و آدرس', icon: 'solar:delivery-outline' },
...
<template>
  <section class="border-y border-divider bg-surface py-4 sm:py-5">
    <ol class="relative mx-auto flex max-w-2xl items-start justify-between px-4 sm:px-8">
      ...
      <li v-for="item in steps" :key="item.id" class="relative z-10 flex w-20 flex-col items-center gap-2 text-center sm:w-28">
        <span class="grid size-10 place-items-center rounded-full border-4 border-surface transition-colors" :class="item.id < step ? 'bg-primary text-primary-foreground' : item.id === step ? 'bg-primary text-primary-foreground ring-2 ring-focus-ring' : 'bg-card text-text-muted'">
          <UIcon :name="item.id < step ? 'solar:check-circle-bold' : item.icon" class="size-5" />
        </span>
