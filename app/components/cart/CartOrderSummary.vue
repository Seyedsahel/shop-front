...
defineProps<{ itemCount: number; subtotalOriginal: number; total: number; disabled?: boolean; discount: number; checkoutLabel?: string }>()
...
<template>
  <aside class="rounded-2xl border border-border bg-card p-5 sm:p-6">
    ...
    <div class="space-y-3 py-5 text-sm">
      <div class="flex items-center justify-between text-text-secondary"><span>قیمت کالاها ({{ itemCount.toLocaleString('fa-IR') }} عدد)</span><span>{{ formatMoney(subtotalOriginal) }}</span></div>
      <div v-if="discount" class="flex items-center justify-between text-success"><span>تخفیف شما</span><span>−{{ formatMoney(discount) }}</span></div>
    ...
    <div class="flex items-center justify-between border-t border-divider pt-4"><span class="font-semibold text-text-primary">مبلغ قابل پرداخت</span><span class="text-lg font-bold text-primary">{{ formatMoney(total) }}</span></div>
    <!-- TODO: Replace coupon validation with Cart API. -->
    <div class="mt-5 flex rounded-xl bg-surface p-1"><input disabled type="text" class="min-w-0 flex-1 bg-transparent px-3 text-xs outline-none placeholder:text-text-muted" placeholder="کد تخفیف یا کارت هدیه" /><button type="button" class="rounded-lg bg-card px-3 py-2 text-xs font-semibold text-text-secondary" disabled>اعمال</button></div>
