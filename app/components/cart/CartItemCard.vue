...
const props = defineProps<{ item: CartUiItem; disabled?: boolean }>()
...
<template>
  <article class="overflow-hidden rounded-2xl border border-border bg-card">
    <div class="flex flex-col gap-4 p-4 sm:flex-row sm:items-center sm:p-5">
      <NuxtLink :to="item.slug ? `/products/${item.slug}` : undefined" class="size-24 shrink-0 overflow-hidden rounded-xl bg-surface sm:size-28">
        <img v-if="item.imageUrl" :src="item.imageUrl" :alt="item.name ?? ''" class="size-full object-cover" />
        <UIcon v-if="!item.imageUrl" name="solar:gallery-outline" class="m-auto size-10 text-text-muted" />
      </NuxtLink>
      <div class="min-w-0 flex-1">
        <div class="mb-2 flex flex-wrap gap-2">
          <UiBadge v-if="hasDiscount" variant="discount">تخفیف ویژه</UiBadge>
          <UiBadge v-if="item.stock !== null" :variant="item.stock > 0 ? 'stock' : 'discount'">{{ item.stock === 0 ? 'ناموجود' : (item.stock ?? 0) > 3 ? 'موجود و آماده ارسال' : `تنها ${item.stock?.toLocaleString('fa-IR')} عدد باقی مانده` }}</UiBadge>
        </div>
        <NuxtLink :to="item.slug ? `/products/${item.slug}` : undefined" class="line-clamp-2 text-sm font-semibold leading-7 text-text-primary hover:text-primary sm:text-base">{{ item.name ?? 'اطلاعات محصول در دسترس نیست' }}</NuxtLink>
        <p v-if="item.presentationUnavailable" class="text-xs text-danger">اطلاعات محصول کامل نیست؛ دریافت سبد را دوباره امتحان کنید.</p>
      ...
      <div class="flex items-center justify-between gap-4 sm:flex-col sm:items-end">
        ...
        <div class="flex h-10 items-center rounded-xl bg-surface p-1">
          <button type="button" class="grid size-8 place-items-center rounded-lg bg-card text-text-secondary transition-colors hover:bg-primary hover:text-primary-foreground disabled:cursor-not-allowed disabled:opacity-40" :disabled="disabled || item.stock === null || item.quantity >= item.stock" aria-label="افزایش تعداد" @click="emit('increase')"><UIcon name="solar:add-circle-outline" class="size-5" /></button>
