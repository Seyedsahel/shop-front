...
const open = () => {
  ...
  navigateTo({ path: `/stories/${props.item.id}`, query: { from: route.fullPath } })
}
</script>

<template>
  <button type="button" class="flex flex-col items-center shrink-0 w-16 snap-start" :aria-label="`مشاهده استوری ${item.title}`" @click="open">
    <div class="size-16 rounded-full p-0.5" :class="seen ? 'bg-border-strong' : 'bg-accent'">
      <div class="size-full rounded-full p-0.5 bg-background">
        <img :src="thumbnailUrl" :alt="item.title" class="size-full rounded-full object-cover mb-1" />
      </div>
    </div>
